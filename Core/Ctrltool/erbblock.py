...
class CheckStack:
    ...
    def line_divider(self):
        ...
        for index_line in enumerate(self.bunch_code):
            ...
            line = line.strip()
            if line.startswith('[SKIPSTART]'): skip_switch = 1
            elif line.startswith('[SKIPEND]'): skip_switch = 0
            elif skip_switch == 1 or line.startswith(';'): continue
            codetype, codeinfo, codeetc = self.code_checker(line)
            if line:
                self.bulkdict[line_cnt] = line
            if sif_switch:
                if codetype == None: pass
                ...
                if (codeinfo, codeetc) == (1,0):
                    ...
                    start_index = start_stack.pop()
                    start_end_que.append((start_index,line_cnt))
            else: raise NotImplementedError(line)
            bef_line_cnt = line_cnt

        for checker in start_end_que:
            if not isinstance(checker,tuple): raise TypeError(checker)
            block_temp = list()
        ...
        if start_stack or crit_stack: print("완성되지 않은 블럭이 있습니다.") #TODO 로그 파일 작성
        return self.bulkdict


class sample_code:
    def __init__(self,target_filename=None):
        if not target_filename: target_filename = input('Input File Name : ')
