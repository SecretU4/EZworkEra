...
class ERBGrammarXML(ImportXML):
    ...
    def __init__(self, filename='config/CustomMarkdown.xml'):
    ...
    def znames_dict(self, origin=None, option_num=0):
        ...
        if option_num in [0, 3]:
            the_list = list(self.tags_callname) + list(self.tags_name)
        elif option_num == 1:
            the_list = self.tags_callname
        elif option_num == 2:
            the_list = self.tags_name
        the_dict = {}
        self.zname_comp_dict = {}
        for zname in the_list:
            if zname in list(self.tags_callname):
                nametag = "callname"
            elif zname in list(self.tags_name):
                nametag = "name"
            else:
                nametag = None
            if origin == None or zname.attrib["origin"] == origin:
                ...
                for particle in part_list:
                    if option_num == 3 and particle.attrib["type"] != "None":
                        continue
    ...
    def zname_dict_situ(self):
        """상황 명칭 변환용 사전"""
        situ_dict = {}
        class_origin_list = self.export_attriblist(self.tags_callname) + self.export_attriblist(
            self.tags_name
        )
...
class SettingXML(ImportXML):
    ...
    def __init__(self, filename='config/EraSetting.xml'):
        super().__init__(filename)
...
class EraLicenceXML(ImportXML):
    def __init__(self, filename='config/EraLicenceFinder.xml'):
        super().__init__(filename)
